  ...
  render: function () {
    ...
    return (
      <button
        ...
        title={this.props.id.toUpperCase()}
      >
  ...
  render: function () {
    ...
    var ind = +this.props.id.slice(4),
      colors = {
        background:
          "hsl(" +
          (50 + Math.floor((Math.floor(ind / 40) + (ind % 60)) * 1.5)) +
          ", 80%, 70%)",
      };
    ...
    return (
      <div
        {...this.props}
        style={
          this.props.randomness == "on"
            ? this.props.alive == "alive"
              ? colors
              : null
            : null
        }
...
var GameBox = React.createClass({
  ...
  componentWillReceiveProps: function () {
    ...
    if (deadcnt == this.props.rows * this.props.cols)
      (playback.paused = "yes"), (playback.restart = "yes");
  ...
  handleClick: function (e) {
    ...
    switch (e.target.id) {
      ...
      case "grid":
        this.setState(function () {
          if (this.state.grid == "off") var ans = "on";
          else ans = "off";
          return { grid: ans };
        });
        return;
      ...
      case "fade":
        if (this.state.fade == "off") var fadeval = "on";
        else fadeval = "off";
        this.setState(function () {
          return { fade: fadeval };
        });
  ...
  render: function () {
    ...
    return (
      ...
      >
        <Panel
          id="panel"
          gen={playback.restart == "no" ? this.props.generation : 0}
        />
        {this.state.list.map(function (data, index) {
          return (
            <Cell
              className={
                "cell cell-" + data + " grid-" + gridval + " fade-" + fadeval
              }
              id={"val-" + index}
              key={index}
              randomness={gridval}
...
(function () {
  ...
  playback.anim = function () {
    ...
    ReactDOM.render(
      <GameBox className="outerbox" generation={time} />,
